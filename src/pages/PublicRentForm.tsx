...
import {
  BIKE_MODELS as DEFAULT_BIKE_MODELS,
  PRICING_RULES as DEFAULT_PRICING_RULES,
  BikeModel,
} from "@/config/bike-rates";
...
export default function PublicRentForm() {
  ...
  const [bikeModels, setBikeModels] =
    useState<BikeModel[]>(DEFAULT_BIKE_MODELS);
  const [pricingRules, setPricingRules] = useState(DEFAULT_PRICING_RULES);

  const [startDate, setStartDate] = useState<string>("");
  const [endDate, setEndDate] = useState<string>("");
  const [selectedModelId, setSelectedModelId] = useState<string>(
    DEFAULT_BIKE_MODELS[0].id,
  );
    ...
    if (effectiveDays >= 30) {
      ...
      if (effectiveDays > pricingRules.longTermDiscountDays) {
        const discountAmount =
          totalLKR * pricingRules.longTermDiscountPercentage;
  ...
  const displayAmount = (amountLKR: number | undefined) => {
    if (amountLKR === undefined || amountLKR === null)
      return currency === "USD" ? "$0.00" : "LKR 0.00";
    if (currency === "USD") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const f of required) {
      if (!String(touristData[f] || "").trim()) {
        setSubmitError(
          `${String(f).replace("_", " ").toUpperCase()} is required`,
        );
        return;
      }
    }
        ...
        <div className="flex-1 flex items-center justify-center p-4 pt-24">
          <div className="max-w-md w-full bg-white rounded-3xl shadow-xl shadow-stone-200/50 p-8 text-center border border-stone-100">
            ...
            <p className="text-stone-600 mb-8">
              Your rental agreement has been submitted successfully. We will
              contact you shortly.
            </p>
            <div className="bg-stone-50 rounded-2xl p-6 mb-8 border border-stone-100">
              <span className="text-xs font-bold text-stone-400 uppercase tracking-widest block mb-1">
                Agreement Reference
              </span>
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col">
      <Helmet>
        <title>Rent a Bike in Tangalle - Sri Rent A Bike Booking</title>
        <meta
          name="description"
          content="Book your scooter or motorbike in Tangalle, Sri Lanka. Easy online booking, transparent pricing, and 24/7 support."
        />
        <link rel="canonical" href="https://www.srirentabike.com/rent" />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://www.srirentabike.com/rent" />
        <meta property="og:site_name" content="Sri Rent A Bike" />
        <meta property="og:locale" content="en_US" />
        <meta
          property="og:title"
          content="Rent a Bike in Tangalle - Sri Rent A Bike Booking"
        />
        <meta
        ...
        <meta name="twitter:url" content="https://www.srirentabike.com/rent" />
        <meta
          name="twitter:title"
          content="Rent a Bike in Tangalle - Sri Rent A Bike Booking"
        />
        ...
        <link
          rel="alternate"
          hreflang="en"
          href="https://www.srirentabike.com/rent"
        />
        <script type="application/ld+json">
          {JSON.stringify({
            "@context": "https://schema.org",
            "@type": "Product",
            name: "Sri Rent A Bike Rental",
            description: "Scooter and motorbike rental in Tangalle, Sri Lanka",
            url: "https://www.srirentabike.com/rent",
            image:
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 pt-24 md:pt-32 pb-12">
        <div className="mb-10 md:mb-14 px-2">
          ...
          <p className="text-stone-600 text-lg md:text-xl max-w-2xl leading-relaxed">
            Experience Tangalle on your own terms. Fill in the details below to
        ...
        <form onSubmit={handleSubmit} className="space-y-8 md:space-y-12">
          ...
          <section className="bg-white rounded-[2rem] shadow-xl shadow-stone-200/40 border border-stone-100 p-6 md:p-10">
            <h2 className="text-2xl font-bold text-stone-900 mb-8 flex items-center gap-3">
              <span className="flex items-center justify-center w-10 h-10 rounded-2xl bg-orange-500 text-white text-base shadow-lg shadow-orange-200">
                1
              </span>
              Bike & Duration
            </h2>
          ...
          <section className="bg-white rounded-[2rem] shadow-xl shadow-stone-200/40 border border-stone-100 p-6 md:p-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              />
              <FormField
                label="Hotel Name in Tangalle"
                name="hotel_name"
                type="text"
                required={false}
                value={touristData.hotel_name}
                onChange={handleInputChange}
                placeholder="The Palm Beach Resort"
                colSpan={true}
              />
            </div>
          </section>
              ...
              <div className="space-y-5 bg-white p-6 md:p-8 rounded-3xl border border-stone-100 shadow-sm">
                <div className="flex justify-between items-center text-stone-600">
                  <span className="text-base font-medium">
                    {selectedModel.name} × {pricing.days} days
                  </span>
                  <span className="font-bold text-stone-900 text-lg">
                    {displayAmount(
                      pricing.totalLKR -
                        (outsideArea
                          ? pricing.days * pricingRules.outsideAreaRateLKR
                          : 0),
                    )}
                  </span>
                </div>
                ...
                <div className="pt-6 border-t-2 border-dashed border-stone-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  ...
                  <div className="text-right w-full sm:w-auto">
                    <div className="text-4xl md:text-5xl font-black text-orange-600 tracking-tight">
                      {displayAmount(pricing.totalLKR)}
                    </div>
                    {currency === "USD" && (
                      <div className="text-xs text-stone-400 font-bold uppercase tracking-widest mt-2 flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-2 hover:bg-stone-100 rounded-lg transition-colors text-orange-500"
                        >
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-2 px-1">
                  ...
                  <p className="text-xs text-stone-500 leading-relaxed font-medium">
                    By signing, you agree to the rental terms and conditions
                    including insurance policies and vehicle return rules.
                  </p>
                </div>
              </div>
        ...
        <footer className="mt-12 text-center">
          <p className="text-stone-400 text-sm">
            Sri Rent A Bike Tangalle • Polommaruwa • +94 77 123 4567
          </p>
        </footer>
      </div>
    </div>
